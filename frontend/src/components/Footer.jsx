...
import { FOOTER_COLUMNS } from "../data/homepage.js";
    ...
    <footer className="border-t border-mist bg-foam px-4 py-12 sm:px-6 lg:px-8">
      <div className="mx-auto grid max-w-6xl gap-10 md:grid-cols-12">
        <div className="md:col-span-6">
          <a href="#top" className="group inline-flex items-center gap-2.5" aria-label="SWEEP home">
          ...
          <div key={column.title} className="md:col-span-3">
            ...
            <ul className="mt-4 space-y-2">
              {column.links.map((link) => (
                <li key={link.label}>
                  <a href={link.href} className="text-sm text-ink/70 transition-all hover:translate-x-1 hover:text-forest">
                    {link.label}
                  </a>
                </li>
              ))}
