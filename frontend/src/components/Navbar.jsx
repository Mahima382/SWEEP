...
import { NAV_LINKS, homeHash } from "../data/homepage.js";
import Button from "./Button.jsx";

/**
 * Site header used on the homepage and every MainLayout page.
 * Transparent over the home hero; solid cream bar everywhere else.
 */
  ...
  return (
    ...
    >
      ...
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
        <Link to="/" className="group flex items-center gap-2.5" aria-label="SWEEP home" onClick={() => setOpen(false)}>
          <span
        ...
        <nav className="hidden items-center gap-1 text-sm font-medium md:flex" aria-label="Primary">
          {NAV_LINKS.map((link) => {
            ...
            return (
              <a
                key={link.href}
                href={homeHash(link.href, isHome)}
        ...
        <div className="hidden items-center gap-3 md:flex">
          <Button href="/login" variant={onHero ? "ghost" : "ghost-dark"}>
            Sign in
          </Button>
          <Button href="/register" variant="forest">
            Join SWEEP
          </Button>
        </div>
      ...
      >
        <nav className="mx-auto flex max-w-6xl flex-col gap-1 px-4 py-4 sm:px-6" aria-label="Mobile">
          {NAV_LINKS.map((link) => (
            <a
              ...
              className={`rounded-2xl px-4 py-3 text-base font-medium transition-colors ${
                isHome && active === link.href ? "bg-lime text-ink" : "hover:bg-sand"
              }`}
          ...
          <div className="mt-3 flex flex-col gap-2 px-1 pb-2">
            <Button href="/login" variant="ghost-dark" className="w-full" onClick={() => setOpen(false)}>
              Sign in
            </Button>
            <Button href="/register" variant="forest" className="w-full" onClick={() => setOpen(false)}>
              Join SWEEP
            </Button>
          </div>
        </nav>
