/**
 * FR-04 CSV and PDF export of the household wallet ledger.
 * PDF is generated in-browser (Helvetica) so no extra package is needed.
 */
 ...
 * @param {{pendingBdt: number, availableBdt: number, earnedBdt: number}} summary Totals.
 * @returns {string} CSV text.
 */
export function buildWalletCsv(transactions, summary) {
 ...
 */
export function buildWalletPdf(transactions, summary) {
  const rows = Array.isArray(transactions) ? transactions : [];
  const lines = [
...
/**
 * Download the wallet ledger as CSV.
 * @param {object[]} transactions Ledger rows.
 * @param {object} summary Wallet totals.
 * @returns {void}
 */
export function downloadWalletCsv(transactions, summary) {
  const csv = buildWalletCsv(transactions, summary);
  downloadBlob(
    `sweep-wallet-${exportStamp()}.csv`,
    new Blob([csv], { type: 'text/csv;charset=utf-8' }),
  );
}

/**
 * Download the wallet ledger as PDF.
 * @param {object[]} transactions Ledger rows.
 * @param {object} summary Wallet totals.
 * @returns {void}
 */
export function downloadWalletPdf(transactions, summary) {
  const pdf = buildWalletPdf(transactions, summary);
  downloadBlob(
    `sweep-wallet-${exportStamp()}.pdf`,
